                                    ...
                                    const D3D12_HEAP_PROPERTIES& heapProps)
{
  ...
  m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufDesc,
                                                  initState, nullptr, IID_PPV_ARGS(&pBuffer)) >> chk;
  return pBuffer;
}
...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
{
  ...
  if (!pCompiler)
  {
    DxcCreateInstance(CLSID_DxcCompiler, __uuidof(IDxcCompiler), (void **)&pCompiler) >> chk;
    DxcCreateInstance(CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void **)&pLibrary) >> chk;
    pLibrary->CreateIncludeHandler(&dxcIncludeHandler) >> chk;
  }
  ...
  pLibrary->CreateBlobWithEncodingFromPinned(
      (LPBYTE)sShader.c_str(), (uint32_t)sShader.size(), 0, &pTextBlob) >> chk;

  // Compile
  IDxcOperationResult* pResult;
  pCompiler->Compile(pTextBlob, fileName, L"", L"lib_6_3", nullptr, 0, nullptr, 0,
                                   dxcIncludeHandler, &pResult) >> chk;

  // Verify the result
  HRESULT resultCode;
  pResult->GetStatus(&resultCode) >> chk;
                                           ...
                                           D3D12_DESCRIPTOR_HEAP_TYPE type, bool shaderVisible)
{
  ...
  device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pHeap)) >> chk;
  return pHeap;
}
